# Claude Code Project Instructions — rama-core-kotlin

## Project Overview

This is **rama-core-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rama-core`](https://crates.io/crates/rama-core).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rama-core/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rama-core>
